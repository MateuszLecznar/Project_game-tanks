...
void check_bomb_collision(Player &hero1,sf::Texture &heroAnimationSheet1,std::vector<sf::Sprite> walls,std::vector<bullet>&bomba,sf::Texture &explosionTexture,std::vector<Boom>&explosia);
void check_bomb_collision1(Player &hero1,sf::Texture &heroAnimationSheet1,std::vector<sf::Sprite> walls,std::vector<bullet>&bomba1,sf::Texture &explosionTexture,std::vector<Boom>&explosia);
int m=0;
int map1=0;
int map2=0;
...
int main() {
    ...
    if (!heroAnimationSheet1.loadFromFile("KV-2_preview.png")) { return -1; };

    sf::Texture explosionTexture;
    if (!explosionTexture.loadFromFile("Explosion.png")) { return -1; };

    sf::Music music;
    if (!music.openFromFile("the high brass.ogg"))
        return -1; // error
    music.setLoop(true);
    music.setVolume(40);
    music.play();
    ...
    sf::SoundBuffer buffer;
      if (!buffer.loadFromFile("De1athFlash.flac"))
         { return -1;}

    sf::SoundBuffer buffer1;
       if (!buffer1.loadFromFile("synthetic_explosion.flac"))
          { return -1;}


       sf::SoundBuffer engine;
          if (!engine.loadFromFile("tank1.wav"))
             { return -1;}
          sf::SoundBuffer engine1;
             if (!engine1.loadFromFile("tank.wav"))
                { return -1;}
    ...
    sf::Texture pocisk1;
    if (!pocisk1.loadFromFile("pocisk.png")) { return -1; };
    sf::Texture pocisk;
    if (!pocisk.loadFromFile("shield.png")) { return -1; };
    pocisk.setSrgb(true);

    sf::Texture win;
    if (!win.loadFromFile("win_1.png")) { return -1; }
    sf::Sprite winner;
   ...
   winner.setPosition(0, 0);
    ...
    sf::Texture front;
    if (!front.loadFromFile("menu.png")) { return -1; }
    sf::Sprite fronte;
